...
```sql
gaggle_search
(query VARCHAR, page INTEGER, page_size INTEGER)
→ VARCHAR
```
...
```sql
SELECT *
FROM json_each(gaggle_search('covid-19', 1, 20));
...
SELECT json_extract_string(value, '$.ref')   as dataset_path,
...
- `json` (VARCHAR): A JSON string to parse and expand
...
```sql
SELECT *
FROM json_each(gaggle_get_version());
